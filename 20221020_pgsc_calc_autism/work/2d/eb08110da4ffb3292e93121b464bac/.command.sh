...
echo nextflow run pgscatalog/pgsc_calc -r v1.2.0 -profile conda --input SPARK_full.csv --target_build GRCh38 --pgs_id PGS000327 --min_overlap 0.01 > command.txt
echo "keep_multiallelic: false" > params.txt
echo "keep_ambiguous   : false"    >> params.txt
echo "min_overlap      : 0.01"       >> params.txt
